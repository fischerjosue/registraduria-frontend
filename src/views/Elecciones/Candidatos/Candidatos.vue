<template>
    ...
    <Teleport to="body">
        <modal :show="ModalInfo" @close="ModalInfo = false">
            <template #header>
                <h3 class="font-bold">Detalles del Candidato</h3>
            </template>
            <template #body>
                <p class="capitalize">Nombre Completo: {{ itemdetalle.nombre }} {{ itemdetalle.apellido }}</p>
                <p class="">Cédula: {{ itemdetalle.cedula }}</p>
                <p class="">Número de resolución: {{ itemdetalle.numero_resolucion }}</p>
            </template>
        </modal>
    </Teleport>
    ...
    <Teleport to="body">
        <modal :show="ModalUpdate" @close="ModalUpdate = false">
            <template #header>
                <h3 class="font-bold">Actualizar Candidato</h3>
            </template>
            <template #body>
                <form v-on:submit.prevent="processItemUpdate(itemMostar._id)">
                    <div>
                        <label for="numero_candidatos"
                            class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Nombre actual:
                            {{ itemMostar.nombre }}</label>
                        <input type="text" id="numero_candidatos" v-model="valor.nombre"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                    </div>
                    <div>
                        <label for="numero_candidatos"
                            class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Apellido actual:
                            {{ itemMostar.apellido }}</label>
                        <input type="text" id="numero_candidatos" v-model="valor.apellido"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                    </div>
                    <div>
                        <label for="cand_insc"
                            class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Cédula actual: {{ itemMostar.cedula }}</label>
                        <input type="text" id="cand_insc" v-model="valor.cedula"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                    </div>
                    <div>
                        <label for="numero_candidatos"
                            class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Resolución actual:
                            {{ itemMostar.numero_resolucion }}</label>
                        <input type="text" id="numero_candidatos" v-model="valor.numero_resolucion"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                    </div>
    ...
    <Teleport to="body">
        <modal :show="ModalNew" @close="ModalNew = false">
            <template #header>
                <h3 class="font-bold">Nuevo Candidato</h3>
            </template>
            <template #body>
                <form v-on:submit.prevent="newItem()">
                    ...
                    <div>
                        <label for="numero_candidatos"
                            class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Resolución:</label>
                        <input type="text" id="numero_candidatos" v-model="valor.numero_resolucion"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                            required>
                    </div>
...
export default {
    ...
    methods: {
        ...
        getData: async function () {
            ...
            const options = {
                method: 'GET',
                url: 'http://127.0.0.1:9999/candidatos',
                headers: {
        ...
        deteleItem: function (user_id) {
            const options = {
                method: 'DELETE',
                url: `http://127.0.0.1:9999/candidatos/${user_id}`,
            };
        ...
        processItemUpdate: function (user_id) {
            axios
                .put(`http://127.0.0.1:9999/candidatos/${user_id}`,
                    this.valor, { headers: {} }
                )
                .then((result) => {
